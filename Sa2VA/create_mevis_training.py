...
# --- Configuration ---
OUTPUT_DIR = 'data/mevis-training'
DATASET_SUBFOLDERS = ['train', 'valid']
...
def main():
    ...
    for dset in DATASET_SUBFOLDERS:
        print(f"\nProcessing dataset: {dset}")
        dset_folder = os.path.join(OUTPUT_DIR, dset)
        ...
        if not all(os.path.exists(p) for p in [tar_path, mask_path, meta_path]):
            ...
            continue
        print(f"Extracting {dset} images...")
        try:
            ...
            continue
        print(f"Loading {dset} JSONs...")
        try:
            with open(mask_path) as f:
                all_masks.update(json.load(f))
            with open(meta_path) as f:
                all_metas.update(json.load(f)['videos'])
        except Exception as e:
    ...
    print(f"Your training data is ready in: {OUTPUT_DIR}")
