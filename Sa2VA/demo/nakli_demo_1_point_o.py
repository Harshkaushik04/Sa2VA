...
if __name__ == "__main__":
    ...
    if cfg.select > 0:
        # --- SINGLE FRAME LOGIC (This worked for you) ---
        img_frame = vid_frames[cfg.select - 1]
        img_path = image_paths[cfg.select - 1] # Get path for the single image
        ...
        # Visualization for the single frame
        if '[SEG]' in prediction and Visualizer is not None:
            _seg_idx = 0
            pred_masks = result['prediction_masks'][_seg_idx]
            pred_mask = pred_masks[0] # Result is for a single image
            if cfg.work_dir:
                os.makedirs(cfg.work_dir, exist_ok=True)
                visualize(pred_mask, img_path, cfg.work_dir)
            else:
                os.makedirs('./temp_visualize_results', exist_ok=True)
                visualize(pred_mask, img_path, './temp_visualize_results')

    else:
        # --- NEW LOOP FOR MULTI-FRAME VIDEO (Fixes OOM) ---
        print(f"Processing {len(vid_frames)} frames one-by-one to save memory...")
        
        # Loop through each frame and its corresponding path
        for i, (img_frame, img_path) in enumerate(zip(vid_frames, image_paths)):
            
            print(f"--- Processing frame {i+1}/{len(vid_frames)} ({os.path.basename(img_path)}) ---")
            
            # Run prediction on ONLY ONE frame
            result = model.predict_forward(
                image=img_frame,  # Use 'image' argument, not 'video'
                text=cfg.text,
                tokenizer=tokenizer,
                # processor=processor,
            ) # type: ignore
            ...
            # Visualization logic MOVED INSIDE the loop
            if '[SEG]' in prediction and Visualizer is not None:
                _seg_idx = 0 # Assuming first segmentation mask
                pred_masks = result['prediction_masks'][_seg_idx]
                pred_mask = pred_masks[0] # The result is for a single image

                if cfg.work_dir:
                    os.makedirs(cfg.work_dir, exist_ok=True)
                    visualize(pred_mask, img_path, cfg.work_dir) # Use the correct img_path
                else:
                    os.makedirs('./temp_visualize_results', exist_ok=True)
                    visualize(pred_mask, img_path, './temp_visualize_results')
            else:
                # This just gives you a warning if you asked for a mask but didn't get one
                if 'segment' in cfg.text.lower(): 
                    print(f"Warning: Segmentation requested but no '[SEG]' token in output for frame {i+1}.")
                pass
    ...
    # --- END OF FILE ---
    # The old prediction/visualization code that was here should be GONE,
    # as it's now handled *inside* the if/else blocks above.
